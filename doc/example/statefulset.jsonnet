...
assert std.length(app.accessPortals) > 0 : "must set one access portal";
...

{
    main: {
        ...
        spec: {
                    ...
                    containers: [
                        {
                            ...
                            [if "format" in one && "specs" in app && std.objectHas(app.specs, one.format) then "resources"]: {
                               ...
                               } + if "requestCustom" in app.specs[one.format] then app.specs[one.format].requestCustom else {},
                               limits: {
                               ...
                               } + if "limitCustom" in app.specs[one.format] then app.specs[one.format].limitCustom else {},
                            },
                            [if "command" in one then "command"]: one.command,
                            [if "args" in one then "args"]: one.args,
            ...
            [if "volumeMounts" in app && std.length(std.filter(function(vol) "VolumeClaimTemplate" == vol.type, app.volumeMounts)) > 0 then "volumeClaimTemplates"]: [
                {
                    metadata: {
                        name: vol.name,
                        namespace: namespace,
                    ...
                    spec: {
                        accessModes: vol.claimTemplate.accessModes,
                        storageClassName: vol.claimTemplate.storageClassName,
                        resources: {
                            requests: {
                                storage: vol.claimTemplate.requestStorage
                            }
                        },
                    }
                } for vol in std.filter(function(vol) "VolumeClaimTemplate" == vol.type && "claimTemplate" in vol, app.volumeMounts)
            ],
        },
    },
